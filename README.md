...
使用 H2 内存数据库 + Spring JdbcTemplate 进行数据持久化，零配置开箱即用，启动时自动建表并初始化样例数据。
```
...
- `schema.sql`、`data.sql`：H2 建表与样例数据
...
| EnrollServiceImpl.java | 重写为 JdbcTemplate 操作数据库 | 原为内存存储，改为真正读写数据库，全栈名副其实 |
...
> 仅需 JDK 17+，无需安装数据库。启动时自动创建 H2 内存数据库并初始化样例数据。
> H2 控制台：http://localhost:8080/h2-console（JDBC URL: `jdbc:h2:mem:enrolldb`，用户名 `sa`，密码留空）
